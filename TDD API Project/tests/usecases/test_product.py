...
async def test_usecases_get_should_return_not_found():
    with pytest.raises(NotFoundException) as err:
        await product_usecase.get(id=UUID("258cca14-4209-48e5-9df1-475857f7b60f"))

    assert (
        err.value.message
        == "Product not found with filter: UUID('258cca14-4209-48e5-9df1-475857f7b60f')"
    )
...
async def test_usecases_delete_should_return_not_found():
    with pytest.raises(NotFoundException) as err:
        await product_usecase.delete(id=UUID("258cca14-4209-48e5-9df1-475857f7b60f"))
